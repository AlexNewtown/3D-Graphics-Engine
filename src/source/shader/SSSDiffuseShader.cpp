...
SSSDiffuseShader::SSSDiffuseShader()
{
	shaderType = SHADER_TYPE_SUBSURFACE_SCATTERING_DIFFUSE;

	addMain(SHADER_SOURCE_SUBSURFACE_SCATTERING_DIFFUSE_VERT, __shaderLinesVert);
	...
	addMain(SHADER_SOURCE_SUBSURFACE_SCATTERING_DIFFUSE_GEOM, __shaderLinesGeom);
	include(SHADER_MATRIX_UTILS, __shaderLinesGeom);

	addMain(SHADER_SOURCE_SUBSURFACE_SCATTERING_DIFFUSE_FRAG, __shaderLinesFrag);
...
SSSDiffuseProjectionTextureShader::SSSDiffuseProjectionTextureShader()
{
	shaderType = SHADER_TYPE_SUBSURFACE_SCATTERING_DIFFUSE_PROJECTION_TEXTURE;

	addMain(SHADER_SOURCE_SUBSURFACE_SCATTERING_DIFFUSE_PROJECTION_TEXTURE_VERT, __shaderLinesVert);
	include(SHADER_MATERIAL_UTILS, __shaderLinesVert);
	include(SHADER_MATRIX_UTILS, __shaderLinesVert);
	//include(SHADER_SH_UTILS, __shaderLinesVert);
	//include(SHADER_RANDOM_UTILS, __shaderLinesVert);
	...
	addMain(SHADER_SOURCE_SUBSURFACE_SCATTERING_DIFFUSE_PROJECTION_TEXTURE_GEOM, __shaderLinesGeom);
	include(SHADER_MATRIX_UTILS, __shaderLinesGeom);

	addMain(SHADER_SOURCE_SUBSURFACE_SCATTERING_DIFFUSE_PROJECTION_TEXTURE_FRAG, __shaderLinesFrag);
	include(SHADER_MATRIX_UTILS, __shaderLinesFrag);
	include(SHADER_GEOMETRY_UTILS, __shaderLinesFrag);
	include(SHADER_MATERIAL_UTILS, __shaderLinesFrag);
	include(SHADER_SHADOW_MAP_UTILS, __shaderLinesFrag);
...
void SSSDiffuseProjectionTextureShader::render()
{
