...
{
	shaderType = SHADER_TYPE_PHOTON_MAP;

	addMain(SHADER_SOURCE_PHOTON_MAP_VERT, __shaderLinesVert);
	include(SHADER_MATRIX_UTILS, __shaderLinesVert);

	addMain(SHADER_SOURCE_PHOTON_MAP_GEOM, __shaderLinesGeom);
	include(SHADER_MATRIX_UTILS, __shaderLinesGeom);

	addMain(SHADER_SOURCE_PHOTON_MAP_FRAG, __shaderLinesFrag);
