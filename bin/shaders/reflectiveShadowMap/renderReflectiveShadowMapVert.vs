...
out vec4 vBasisX;
out vec4 vBasisZ;
out vec4 vEntityPosition;
out vec4 vEntityNormal;
...
void main()
{
	...
	vPosition = eyeCoords;
	vEntityPosition =  matrixMult(entityMatrix, position);
	vColor = color;
	...
	vEntityNormal = matrixMult(entityNormalMatrix, normal);
	vEntityNormal = vec4(normalize(vec3(vEntityNormal)),1.0);
